...
{
public:
    track(const String& file_path, AudioTransportSource& transport_source) :
    ...
    void set_on_mouse_event(const std::function<void(track*, bool double_click)>& callback) {
        _callback_mouse_event = callback;
    }

    void set_on_close(const std::function<void(track*)>& callback) {
...
private:
    ...
    std::function<void(track*, bool)> _callback_mouse_event;
    std::function<void(track*)>       _callback_close;
